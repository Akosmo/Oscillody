...
func set_value(p_value: float) -> Error:
	return _set_value(p_value)

func set_value_no_signal(p_value: float) -> Error:
	return _set_value(p_value, false)

func _set_value(p_value: float, p_signal: bool = true) -> Error:
	if (
	...
	if p_signal:
		...
		_h_slider.set_value_no_signal(p_value)
	_set_value_to_label()
