...
var _configuration_resource: ElementUIConfigurations
...
func _ready() -> void:
	...
	_property_label.set_text(_property_key_no_substr)
	
	set_name(_property_label.get_text().replacen(" ", ""))
	
	_configuration_resource = ElementManager.get_element_ui_configurations(element.get_type())
...
func _connect_signals() -> void:
	...
	ElementManager.element_created.connect(_on_external_options_changed)
	ElementManager.element_deleted.connect(_on_external_options_changed)
	ElementManager.property_node_visibility_changed.connect(_on_property_visibility_changed)
	AudioManager.stream_list_updated.connect(_on_external_options_changed_no_parameter)
	
	_reset_button.pressed.connect(_on_reset_pressed)
	
	_button.pressed.connect(_on_button_pressed)
	_file_dialog.file_selected.connect(_on_file_selected)
	_check_button.toggled.connect(_on_check_toggled)
	...
	_custom_h_slider.value_changed.connect(_on_slider_value_changed)
	_option_button.item_selected.connect(_on_option_item_selected)
...
func _set_property_value_to_control() -> void:
	...
	#match _control_node:
	if _control_node == ElementUIConfigurations.ControlNode.BUTTON:
		_button.show()
		_button.set_text("Select Image")
	elif _control_node == ElementUIConfigurations.ControlNode.CHECK_BUTTON:
		_check_button.show()
		@warning_ignore("unsafe_cast")
		_check_button.set_pressed(_property_value as bool)
	elif _control_node == ElementUIConfigurations.ControlNode.COLOR_PICKER_BUTTON:
		_color_picker_button.show()
		@warning_ignore("unsafe_cast")
		_color_picker_button.set_pick_color(_property_value as Color)
	elif _control_node == ElementUIConfigurations.ControlNode.LINE_EDIT:
		_line_edit.show()
		_line_edit.set_text(str(_property_value))
	elif _control_node == ElementUIConfigurations.ControlNode.NUMERICAL:
	...
	elif _control_node == ElementUIConfigurations.ControlNode.OPTION_BUTTON:
		_option_button.show()
	...
	elif _control_node == ElementUIConfigurations.ControlNode.TEXT_EDIT:
		_text_edit.show()
...
func _update_property_configuration_dictionary() -> void:
	...
	if element.get_type() == element.ElementType.EMPTY:
		#var config_resource: ElementEmptyUIConfigurations = preload("uid://c5noed3x5ibew")
		var all_configs: Dictionary[StringName, Dictionary] = \
		_configuration_resource.get_property_configurations()
		var property_dict: Dictionary = all_configs.get(property_key)
		_property_configurations = property_dict
	elif element.get_type() == element.ElementType.ANALYZER:
		#var config_resource: ElementAnalyzerUIConfigurations = preload("uid://blysgijw0k6ah")
		var all_configs: Dictionary[StringName, Dictionary] = \
		_configuration_resource.get_property_configurations()
		var property_dict: Dictionary = all_configs.get(property_key)
		_property_configurations = property_dict
	elif element.get_type() == element.ElementType.IMAGE:
		#var config_resource: ElementImageUIConfigurations = preload("uid://d22vhxv5sqx2t")
		var all_configs: Dictionary[StringName, Dictionary] = \
		_configuration_resource.get_property_configurations()
		var property_dict: Dictionary = all_configs.get(property_key)
		_property_configurations = property_dict
	elif element.get_type() == element.ElementType.SHADER:
		#var config_resource: ElementShaderUIConfigurations = preload("uid://ccdqa8ifs0fso")
		#var all_configs: Dictionary[StringName, Dictionary] = \
		#_configuration_resource.get_property_configurations()
		#var property_dict: Dictionary = all_configs.get(property_key)
		_property_configurations = _configuration_resource.get_property_configurations_3(property_key)
	elif element.get_type() == element.ElementType.TEXT:
		pass
	elif element.get_type() == element.ElementType.VISUAL_EFFECT:
		pass
